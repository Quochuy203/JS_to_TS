...
export function moyenne(note: number[]): number | string {
  if (note.length === 0) return "Aucune note";
  const total = note.reduce((a, b) => a + b, 0);
...
//trierPar(FILMS, "titre"); (Bonus , avance)
...
//filtrerParGenre(FILMS)
//filtrerParGenre(FILMS, "SF"); (Bonus , avance)
